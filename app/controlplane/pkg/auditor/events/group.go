...
const (
	...
	GroupMemberUpdatedActionType string             = "GroupMemberUpdated"
)
...
// GroupMemberUpdated represents the update of a member's status in a group
type GroupMemberUpdated struct {
	*GroupBase
	UserID        *uuid.UUID `json:"user_id,omitempty"`
	UserEmail     string     `json:"user_email,omitempty"`
	NewMaintainer *bool      `json:"new_maintainer,omitempty"`
}

func (g *GroupMemberUpdated) ActionType() string {
	return GroupMemberUpdatedActionType
}
...
func (g *GroupMemberUpdated) ActionInfo() (json.RawMessage, error) {
	...
	if g.NewMaintainer == nil {
		return nil, fmt.Errorf("new maintainer status is required")
	}
...
func (g *GroupMemberUpdated) Description() string {
	maintainerStatus := "not a maintainer"
	if *g.NewMaintainer {
		maintainerStatus = "a maintainer"
	}

	return fmt.Sprintf("{{ if .ActorEmail }}{{ .ActorEmail }}{{ else }}API Token {{ .ActorID }}{{ end }} has updated user %s to be %s in the group %s",
		g.UserEmail, maintainerStatus, g.GroupName)
}
