...
func (g GroupRepo) ListMembers(ctx context.Context, orgID uuid.UUID, groupID uuid.UUID, opts *biz.ListMembersOpts, paginationOpts *pagination.OffsetPaginationOpts) ([]*biz.GroupMembership, int, error) {
	...
	query := g.data.DB.GroupUser.Query().
		Where(groupuser.GroupID(groupID), groupuser.DeletedAtIsNil())

	if opts != nil && opts.Maintainers != nil {
		query.Where(groupuser.MaintainerEQ(*opts.Maintainers))
	}

	if opts != nil && opts.MemberEmail != nil {
		query.Where(groupuser.HasUserWith(user.EmailContains(*opts.MemberEmail)))
	}
	...
	for _, member := range members {
		bizMembers = append(bizMembers, entGroupUserToBiz(member))
	}
...
func (g GroupRepo) Create(ctx context.Context, orgID uuid.UUID, opts *biz.CreateGroupOpts) (*biz.Group, error) {
	...
	err := WithTx(ctx, g.data.DB, func(tx *ent.Tx) error {
		...
		if _, grUerr := tx.GroupUser.Update().
			Where(
				groupuser.GroupIDEQ(gr.ID),
				groupuser.UserIDEQ(opts.UserID),
			).
			SetMaintainer(true).
			Save(ctx); grUerr != nil {
...
// entGroupUserToBiz converts an ent.GroupUser to a biz.GroupMembership.
func entGroupUserToBiz(gu *ent.GroupUser) *biz.GroupMembership {
